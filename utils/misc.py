...
def cos_gram_matrix(feat):
    ...
    # mul
    inner = torch.bmm(feat, feat_t) # or: matmul
    # norm
    feat_l2 = 1. / torch.norm(feat, dim=2)
    feat_norm = torch.matmul(feat_l2.unsqueeze(2), feat_l2.unsqueeze(1))
    # cosine Gram matrix
    cos_gram = inner.mul(feat_norm)
